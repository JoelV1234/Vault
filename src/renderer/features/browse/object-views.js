...
const rowMenu = (b, o) => (e) => {
  ...
  dropdown(e.currentTarget, [
    ...
    '-',
    { label: 'Move to trash', icon: 'trash-2', danger: true, onClick: deleteObject(b, o) },
  ]);
};

// Hover trash control; rows are <button>s, so this is a span with button role.
export const rowTrashBtn = (label, onDelete) => el('span', {
  class: 'row-trash', role: 'button', tabindex: '0', 'aria-label': label, title: label,
  onclick: (e) => { e.stopPropagation(); onDelete(); },
  onkeydown: (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); onDelete(); }
  },
}, icon('trash-2', 14));
...
export function renderList(b, list) {
  ...
  return el('div', { class: 'obj-list' }, list.map((o) =>
    el('button', { class: 'obj-row lift', onclick: () => open(o.id), oncontextmenu: rowMenu(b, o) },
      ...
      rowTrashBtn(`Move "${o.title}" to trash`, deleteObject(b, o)))));
}
...
export function renderTable(b, list) {
  ...
  return el('div', { class: 'table-wrap' }, el('table', { class: 'obj-table' },
    ...
    el('tbody', {}, list.map((o) => el('tr', { onclick: () => open(o.id), tabindex: '0', role: 'button',
      ...
      el('td', { class: 'td-trash' }, rowTrashBtn(`Move "${o.title}" to trash`, deleteObject(b, o))))))));
}
